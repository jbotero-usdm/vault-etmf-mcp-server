...
export default async function handler(req, res) {
  ...
  const status = overview.overview?.health === 'green' ? 'ok' : overview.overview?.health === 'amber' ? 'warning' : 'error';
  ...
  res.status(200).json({
    ...
    environment: {
      ...
      name: 'SmartTMF Box → Vault via Glean',
      mode: 'live'
    },
    datasources: {
      documents: {
        ...
        description: 'Vault documents exposed through Glean'
      },
      objects: {
